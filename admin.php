...
<?php
	...
	if(isset($_POST['modiflvl'])){
		...
		$sql= "UPDATE level SET state='{$state}', text_book='{$text_book}', focus_on='{$focus_on}', comprehension='{$comprehension}' WHERE id_student ={$id}";
		$db->query($sql);
	} 	// Modify level
	...
	if(isset($_POST['AddPass'])){
		...
		$pass=filtrarResU8($_POST['pass']);
		$password=md5($pass);
		addpass($id_student, $password);
	}		// Add password
...
<head>
	...
	<script>
		...
		function inicio() {
			...
			document.getElementById('AddPass').onclick = AddPass;
		...
		function CloseTables() {
			...
			document.getElementById('BoxAddPass').style.display = 'none';
		...
		function AddPass() {
			CloseTables();
			document.getElementById('addAssistance').style.display = 'none';
			document.getElementById('BoxAddPass').style.display = 'flex';
		} // Show form pass
...
<body>
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item dropdown">
							...
							<div class="dropdown-divider"></div>
								<button class="dropdown-item" id='showInStudents'>MODIFY CLASS</button>
							</div>
						</li>
						<li class="nav-item">
						...
						<li class="nav-item dropdown">
							...
							<div class="dropdown-menu" aria-labelledby="navbarDropdown">
								...
								<button class="dropdown-item" id='AddPass'>PASSWORD</button>
							</div>
						</li>
						<li class="nav-item">
	...
	<div class="container p-0">
		<div class="row d-flex justify-content-center">
			...
			<div id='tablaStudents' class="overflow-auto col-auto">
				<?php tableStudentsEdit('!=', 'table-success', 'bg-success'); ?>
			</div> <!-- Tabal estudiantes editable -->
			<div id='tablaInStudents' class="overflow-auto col-auto">
				<?php tableStudentsEdit('=', 'table-danger','bg-danger'); ?>
	...
	<div class="container d-flex justify-content-center mt-3">
		...
		<div id="BoxAddPass" style="width: 300px">
			<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
				<legend>SET STUDENT PASSWORD </legend>
				<div class="form-group row d-flex align-items-center">
					<div class="col-12">
						<label class="m-0" for="">Student</label>
						<?php 
					...
					<div class="col-12">
						...
						<input class="form-control m-0" type="password" name="pass" placeholder="pasword" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}">
						<small id="emailHelp" class="form-text text-muted">Password must contain 8 or more characters that are of at least one number, and one uppercase and lowercase letter</small>
					</div>
				</div>

				<div class="row">
					<button type="submit" class="btn btn-primary" name="AddPass" style="width: 100%">ADD</button>
				</div>
			</form>
		</div><!-- ADD PASS -->
	</div> <!-- Formularios Add Student, Add Level y Add Pass -->
	<div class="container d-flex justify-content-center mt-3">
